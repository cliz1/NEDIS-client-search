...
#Builds the client search directory
    ...
    for row in data:
        if row[headers.index('Choose Form Type')] == "Intake":
            ...
            'Students': [row[headers.index('Student first name')]+" "+row
                         [headers.index('Student last name')]+" Grade"+row[headers.index('Current grade')]],
            'Services': {}}
        ...
        if row[headers.index('Choose Form Type')] == "Advocacy":
...
def search():
    ...
    return render_template('search_results.html', matching_clients=matching_clients)
